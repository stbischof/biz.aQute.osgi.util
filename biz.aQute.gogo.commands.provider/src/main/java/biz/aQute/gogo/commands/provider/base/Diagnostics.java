...
public class Diagnostics implements Closeable {
	...
	public Diagnostics(BundleContext context, DTOFormatter dtof) {
	...
	public List<Requirement> reqs(@Descriptor("Only show the requirements of the given bundle")
	@Parameter(names = {
		"-b", "--bundle"
	}, absentValue = "*")
	Glob bundle,
		@Descriptor("Only show the requirements when one of the given namespace matches. You can use wildcards.")
		Glob... ns) {
	...
	private boolean matches(Glob[] ns, String namespace) {
		if (ns == null || ns.length == 0)
			return true;

		for (Glob g : ns) {
			if (g.finds(namespace) >= 0)
				return true;
		}
		return false;
	}
	...
	public List<Capability> caps(@Descriptor("Only show the capabilities of the given bundle")
	@Parameter(names = {
		"-b", "--bundle"
	}, absentValue = "-1")
	long bundle,
	...
	public List<Search> wanted(@Descriptor("If specified will only show for the given bundle")
	@Parameter(names = {
		"-b", "--bundle"
	}, absentValue = "-1")
	long exporter,
		@Descriptor("If specified, this glob expression must match the name of the service class/interface name")
		@Parameter(names = {
			"-n", "--name"
		}, absentValue = "*")
		Glob name) throws InvalidSyntaxException {
	...
	public Collection<Export> exports(@Descriptor("If specified will only show for the given bundle")
	@Parameter(names = {
		"-b", "--bundle"
	}, absentValue = "-1")
	long exporter,
		...
		Glob name, @Descriptor("Show all packages, not just the ones that look fishy")
		@Parameter(names = {
			"-a", "--all"
		}, absentValue = "false", presentValue = "true")
		boolean all, @Descriptor("Check exports against private packages")
		@Parameter(names = {
			"-p", "--private"
		}, absentValue = "false", presentValue = "true")
		boolean privatePackages) {
		...
		for (Capability c : caps) {
			...
			org.osgi.resource.Resource resource = c.getResource();
