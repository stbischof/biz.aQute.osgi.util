...
public class Files {
	...
	public File cd(CommandSession session, @Descriptor("target directory")
	String dir) throws IOException {
		File cwd = pwd(session);
		if ((dir == null) || (dir.length() == 0)) {
			return cwd;
		}
		cwd = IO.getFile(cwd, dir);
	...
	public File[] ls(CommandSession session, @Descriptor("path with optionally wildcarded file name")
	String pattern) throws IOException {
		...
		File actualParent = ((parent.charAt(0) == File.separatorChar) ? new File(parent)
			: new File(cd(session), parent)).getCanonicalFile();
